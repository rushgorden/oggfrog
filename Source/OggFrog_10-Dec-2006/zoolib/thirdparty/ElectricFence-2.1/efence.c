...
ag@zoolib.org 2002-11-03. I've tweaked the code such that access is protected by
a global pthread mutex. We have to use a recursive mutex because malloc is
built atop memalign, which calls allocateMoreSlots which in turn can call malloc.
*/
...
#define __USE_GNU
#include <pthread.h>
//pthread_mutex_t sMutex = PTHREAD_RECURSIVE_MUTEX_INITIALIZER_NP;
...
malloc(size_t size)
{
	...
	void* result = memalign(EF_ALIGNMENT, size);
	pthread_mutex_unlock(&sMutex);
	return result;
}
...
{
	pthread_mutex_lock(&sMutex);
	void* result = memalign(bytesPerPage, size);
	pthread_mutex_unlock(&sMutex);
	return result;
}
