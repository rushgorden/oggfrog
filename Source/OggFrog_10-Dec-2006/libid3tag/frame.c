				  ...
				  unsigned int version)
{
  ...
  if (ID3_TAG_VERSION_MAJOR(version) < 4) {
	...
	goto fail;
      ...
      compat = id3_compat_lookup( (const char*)id, 3);	// MDC
	...
	goto fail;
      ...
      compat = id3_compat_lookup( (const char*)id, 4);	// MDC
	...
	frame = unparseable( (const char*)id, ptr, end - *ptr, 0, 0, 0, 0); // MDC
	goto done;
      }
    ...
    if (compat && compat->equiv)
      id = (id3_byte_t*)compat->equiv;					// MDC
